  ...
  const getBeritaById = async (beritaId: number) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:4000/api/peraturan/${beritaId}`,
        {
...
export function useGetRuleByDesa() {
  ...
  const getRuleByDesa = async (desa: string) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:4000/api/peraturan/desa/${desa}`,
        {
          withCredentials: true,
        }
      );
  ...
  const uploadDocument = async (file: File) => {
    ...
    try {
      ...
      const res = await axios.post(
        "http://localhost:4000/api/peraturan/upload-docs",
        formData,
        {
          withCredentials: true,
          headers: {
...
export function useCreateNewRule() {
  ...
  const createRule = async (data: {
    ...
    try {
      const res = await axios.post(
        "http://localhost:4000/api/peraturan/upload",
        data,
        {
          withCredentials: true,
          headers: {
